<template>
  <div class="poly-view">
    <nav class="navbar">
      ...
      <span class="nav-sub">Polymarket</span>
    ...
    <div class="layout">
      ...
      <aside class="sidebar">
        ...
        <div v-if="tab === 'catalog'" class="market-list">
          ...
          <div
            v-for="m in markets"
            :key="m.id || m.market_id"
            class="market-item"
            :class="{ selected: selectedMarket && (selectedMarket.id || selectedMarket.market_id) === (m.id || m.market_id) }"
            @click="selectMarket(m)"
          >
            <div class="market-item-question">{{ m.question }}</div>
            <div class="market-item-meta">
              <span class="meta-price yes-price">YES {{ pct(m.yes_price) }}</span>
              <span class="meta-vol" v-if="m.volume">{{ fmtVol(m.volume) }}</span>
              <span class="meta-cat">{{ m.category }}</span>
            </div>
          </div>
          <button v-if="markets.length >= offset" class="load-more-btn" @click="loadMore">Load more</button>
        </div>
        ...
        <div v-if="tab === 'scan'" class="market-list">
          ...
          <div
            ...
            :class="{ selected: selectedMarket && selectedMarket.market_id === o.market_id }"
            @click="selectOpportunity(o)"
          >
            <div class="market-item-question">{{ o.question }}</div>
            <div class="market-item-meta">
              <span class="meta-edge" :class="edgeClass(o.edge_signal)">{{ o.edge_signal }}</span>
              <span class="meta-edge-val">{{ pctSigned(o.edge) }}</span>
            </div>
          </div>
        </div>
        ...
        <div v-if="tab === 'portfolio'" class="portfolio-panel">
          ...
          <div v-else>
            <div class="portfolio-section">
              ...
              <div v-for="o in portfolio.orders" :key="o.order_id || o.id" class="portfolio-row">
                <span class="portfolio-ticker">{{ o.market_id?.slice(0, 10) }}…</span>
                <span class="portfolio-side" :class="sideClass(o.side)">{{ o.side }}</span>
                <span>${{ o.amount?.toFixed(2) }}</span>
                <span class="portfolio-status" :class="{ 'status-dry': o.status === 'dry_run_accepted' }">
                  {{ o.status === 'dry_run_accepted' ? 'DRY RUN' : o.status }}
                </span>
              </div>
            ...
            <div class="portfolio-section">
              ...
              <div v-for="p in portfolio.positions" :key="p.market_id" class="portfolio-row">
                <span class="portfolio-ticker">{{ p.market_id?.slice(0, 14) }}…</span>
                <span>{{ p.side }}</span>
                <span>${{ p.size }}</span>
              </div>
      ...
      <main class="detail-panel">
        ...
        <div v-else>
          ...
          <div class="detail-header">
            <div class="detail-ticker">{{ selectedMarket.id || selectedMarket.market_id }}</div>
            <div class="detail-question">{{ selectedMarket.question }}</div>
            <div class="detail-meta-row">
              <span class="detail-cat">{{ selectedMarket.category }}</span>
              <span class="detail-close" v-if="selectedMarket.days_to_close != null">{{ selectedMarket.days_to_close }}d to close</span>
              <span class="detail-vol" v-if="selectedMarket.volume">Vol: {{ fmtVol(selectedMarket.volume) }}</span>
              <span class="detail-traders" v-if="selectedMarket.num_traders">{{ selectedMarket.num_traders }} traders</span>
            </div>
          </div>
          ...
          <div class="price-bar">
            ...
            <div class="price-block no-block">
              ...
              <div class="price-val no-val">{{ pct(selectedMarket.no_price ?? (1 - (selectedMarket.yes_price || 0.5))) }}</div>
            </div>
            <div class="liquidity-block" v-if="selectedMarket.liquidity">
              <div class="price-label">LIQUIDITY</div>
              <div class="liquidity-val">{{ fmtVol(selectedMarket.liquidity) }}</div>
            </div>
          </div>

          <!-- Outcomes (multi-outcome markets) -->
          <div v-if="selectedMarket.outcomes?.length > 2" class="outcomes-section">
            <div class="section-tag">OUTCOMES</div>
            <div class="outcome-row" v-for="(oc, i) in selectedMarket.outcomes" :key="i">
              <span class="outcome-name">{{ oc.name }}</span>
              <div class="outcome-bar-wrap">
                <div class="outcome-bar" :style="{ width: pct(oc.price) }"></div>
              </div>
              <span class="outcome-prob">{{ pct(oc.price) }}</span>
            </div>
          </div>
          ...
          <div class="prediction-section">
            ...
            <button class="predict-btn" :disabled="predicting" @click="runPrediction">
            ...
            <div v-if="prediction" class="prediction-result">
              <div class="pred-row">
                ...
                <div class="pred-block">
                  ...
                  <div class="pred-val" :class="['edge-val', edgeClass(prediction.edge_signal)]">
          ...
          <div class="trade-section">
            <div class="section-tag">
              ...
              <span class="mode-badge">{{ executorMode?.toUpperCase() || 'REST' }}</span>
            </div>
            ...
            <div class="trade-form">
              ...
              <div class="form-row">
                ...
                <div class="form-field">
                  ...
                  <input v-model.number="tradeForm.price" type="number" min="0.01" max="0.99" step="0.01" class="form-input"
                    :placeholder="(selectedMarket.yes_price || 0.5).toFixed(2)" />
                </div>
                <div class="form-field">
              ...
              <div class="form-summary" v-if="tradeForm.amount && tradeForm.price">
                Sized: <strong>${{ (tradeForm.amount * (tradeForm.kelly || 0.25)).toFixed(2) }} USDC</strong>
                @ {{ ((tradeForm.price || selectedMarket.yes_price || 0.5) * 100).toFixed(0) }}¢
              </div>
            ...
            <div v-if="tradeResult" class="trade-result" :class="{ 'result-ok': !tradeResult.error, 'result-err': tradeResult.error }">
              <div class="result-status">
                {{ tradeResult.error ? 'FAILED' : (tradeResult.status === 'dry_run_accepted' ? 'DRY RUN ACCEPTED' : 'ORDER PLACED') }}
              </div>
              <div class="result-id" v-if="tradeResult.order_id">ID: {{ tradeResult.order_id }}</div>
              <div class="result-detail">
                {{ tradeResult.side }} · ${{ tradeResult.amount?.toFixed(2) }} USDC · {{ pct(tradeResult.price) }}
              </div>
...
import {
  getPolymarketCatalog, searchPolymarketMarkets, getPolymarketMarket,
  predictPolymarket, scanPolymarketOpportunities, placePolymarketTrade,
  getPolymarketOrders, getPolymarketPositions, getPolymarketStatus
} from '../api/polymarket'
...
const statusLabel = computed(() => {
  ...
  const e = executorStatus.value.executor
  if (!e) return 'Unknown'
  if (e.connected) return e.dry_run ? `DRY RUN (${e.mode})` : e.mode.toUpperCase()
  return e.has_private_key ? 'Disconnected' : 'No Credentials'
})
...
async function loadCatalog(reset = false) {
  if (reset) { markets.value = []; offset.value = 0 }
  marketsLoading.value = true
  try {
    const res = await getPolymarketCatalog(limit, offset.value)
    markets.value = [...markets.value, ...(res.data?.markets || [])]
    offset.value += (res.data?.markets?.length || 0)
  } catch (e) {
    console.error(e)
  } finally {
...
async function doSearch() {
  ...
  marketsLoading.value = true
  try {
    const res = await searchPolymarketMarkets(searchQuery.value)
...
async function scanOpportunities() {
  ...
  opportunities.value = []
  try {
    const res = await scanPolymarketOpportunities()
...
async function loadPortfolio() {
  ...
  try {
    const [ord, pos] = await Promise.allSettled([getPolymarketOrders(), getPolymarketPositions()])
...
async function selectMarket(m) {
  ...
  try {
    const res = await getPolymarketMarket(id)
...
async function runPrediction() {
  ...
  try {
    ...
    const res = await predictPolymarket({
      market_id: m.id || m.market_id,
      question: m.question,
      yes_price: m.yes_price ?? 0.5,
      no_price: m.no_price ?? (1 - (m.yes_price ?? 0.5)),
      volume: m.volume ?? 0,
      category: m.category ?? 'unknown',
      days_to_close: m.days_to_close ?? 30,
    })
...
async function placeTrade() {
  ...
  try {
    ...
    const body = {
      ...
      price: tradeForm.value.price || m.yes_price,
      kelly_fraction: tradeForm.value.kelly || 0.25,
    }
    const res = await placePolymarketTrade(body)
...
async function checkStatus() {
  try {
    const res = await getPolymarketStatus()
    executorStatus.value = res.data
    dryRunMode.value = res.data?.executor?.dry_run ?? true
    executorMode.value = res.data?.executor?.mode ?? 'rest'
  } catch (e) {
    console.error(e)
  }
}
...
.list-loading, .list-empty { padding: 24px 16px; font-size: 0.8rem; color: #999; }
...
.market-item-meta { display: flex; gap: 8px; align-items: center; flex-wrap: wrap; }

.meta-price { font-size: 0.65rem; font-weight: 700; padding: 2px 6px; }
.yes-price { background: #dcfce7; color: #166534; }
.meta-vol { font-size: 0.65rem; color: #6b7280; }
.meta-cat { font-size: 0.65rem; color: #999; text-transform: uppercase; letter-spacing: 1px; }

.meta-edge { font-size: 0.65rem; font-weight: 700; padding: 2px 6px; }
...
.portfolio-panel { flex: 1; overflow-y: auto; padding: 12px 16px; }
.portfolio-section { margin-bottom: 20px; }
.portfolio-label { font-size: 0.6rem; font-weight: 700; letter-spacing: 2px; color: #999; margin-bottom: 8px; }
...
.portfolio-ticker { font-weight: 700; flex: 1; }
...
.empty-icon { font-size: 2.5rem; color: #ccc; }
...
.detail-header { border-bottom: 2px solid #000; padding-bottom: 16px; }

.detail-ticker { font-size: 0.7rem; font-weight: 700; letter-spacing: 2px; color: #999; margin-bottom: 6px; word-break: break-all; }
.detail-question { font-size: 1.1rem; font-weight: 700; line-height: 1.4; margin-bottom: 10px; }
.detail-meta-row { display: flex; gap: 16px; font-size: 0.75rem; color: #666; flex-wrap: wrap; }
...
.price-block { flex: 1; min-width: 80px; display: flex; flex-direction: column; align-items: center; gap: 4px; }
.price-label { font-size: 0.6rem; font-weight: 700; letter-spacing: 2px; color: #999; }
.price-val { font-size: 2rem; font-weight: 700; color: #22c55e; }
.no-val { color: #ef4444; }
.price-divider { font-size: 0.7rem; font-weight: 700; color: #ccc; }
.liquidity-block { display: flex; flex-direction: column; align-items: center; gap: 4px; }
.liquidity-val { font-size: 0.95rem; font-weight: 700; color: #666; }

.outcomes-section { border: 1px solid #e5e5e5; padding: 16px; display: flex; flex-direction: column; gap: 10px; }
.outcome-row { display: flex; align-items: center; gap: 10px; }
.outcome-name { font-size: 0.8rem; font-weight: 600; width: 140px; }
.outcome-bar-wrap { flex: 1; height: 6px; background: #f0f0f0; }
.outcome-bar { height: 100%; background: #000; transition: width 0.4s; }
.outcome-prob { font-size: 0.8rem; font-weight: 700; width: 40px; text-align: right; }
...
.pred-row { display: flex; gap: 24px; align-items: flex-end; flex-wrap: wrap; }
...
.pred-val { font-size: 1.4rem; font-weight: 700; }
.yes-val { color: #22c55e; }

.pred-signal { font-size: 0.75rem; font-weight: 700; padding: 3px 8px; letter-spacing: 1px; }
...
.pred-conf { font-size: 0.65rem; font-weight: 700; padding: 3px 8px; letter-spacing: 1px; }
.conf-high { background: #22c55e; color: #000; }
.conf-medium { background: #f59e0b; color: #000; }
.conf-low { background: #ef4444; color: #fff; }

.pred-reasoning { font-size: 0.8rem; line-height: 1.6; color: #444; border-left: 2px solid #FF4500; padding-left: 10px; }
...
.dry-badge { font-size: 0.6rem; font-weight: 700; padding: 2px 8px; background: #fef3c7; color: #92400e; letter-spacing: 1px; }
.mode-badge { font-size: 0.6rem; font-weight: 700; padding: 2px 8px; background: #e0e7ff; color: #3730a3; letter-spacing: 1px; }
...
.trade-result { padding: 14px; border-left: 3px solid #22c55e; background: #f0fdf4; }
.trade-result.result-err { border-left-color: #ef4444; background: #fef2f2; }
.result-status { font-size: 0.65rem; font-weight: 700; letter-spacing: 2px; margin-bottom: 4px; }
.result-id { font-size: 0.75rem; color: #666; margin-bottom: 4px; word-break: break-all; }
